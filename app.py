...
st.markdown(
    ...
    **请通过左侧导航栏选择页面：**
    ...
    | 🧭 研究雷达 | 提炼上升主题、研究缺口、假设卡、复现执行计划、方法信号、推荐论文，并支持专题工作台生成综述草稿 |
    """
)
...
with st.expander(f"📋 版本更新日志（当前：{APP_VERSION}）", expanded=False):
    st.markdown(
        ...
        **新增 / 优化**
        ...
        - 新增执行计划 Markdown 导出，支持将选题快速推进为可落地、可复现的研究项目

        ### v1.9 — 2026-06-08
        **新增 / 优化**
        - 研究雷达新增「🧪 研究假设实验室」：将缺口地图转化为可检验假设卡，自动生成处理变量、结果变量、识别方案、稳健性和证伪测试
        - 新增假设实验室 Markdown 导出，便于形成可执行研究提案草稿

        ### v1.8 — 2026-06-08
        **新增 / 优化**
        - 研究雷达新增「🕳️ 研究缺口地图」：自动构建「领域 × 方法」矩阵，发现活跃领域与活跃方法之间尚少交叉的潜在选题
        - 新增缺口问题库与数据线索提示，并支持 Markdown 导出，便于形成研究计划草稿
